...
func (e *SysUser) SetPwd(pwd SysUserPwd) (Result bool, err error) {
	...
	if err != nil {
		security.HasError(err, "获取用户数据失败(代码202)", 500)
	}
	_, err = security.CompareHashAndPassword(user.Password, pwd.OldPassword)
	if err != nil {
		if strings.Contains(err.Error(), "hashedPassword is not the hash of the given password") {
			security.HasError(err, "密码错误(代码202)", 500)
		}
		log.Print(err)
		return
	}
	...
	security.HasError(err, "更新密码失败(代码202)", 500)
	return
}
