...
import (
	"context"
	"fmt"
	tools2 "github.com/panda8z/istone/src/pkg/tools"
	"log"
	"net"
	"net/http"
	"net/url"
...
func Start() {
	...
	appdashPort := 8700
	appdashURLStr := fmt.Sprintf("http://%s:%d", tools2.GetLocaHonst(), appdashPort)
	...
	fmt.Println(tools2.Green("Trace server run at:"))
	fmt.Printf("- Local: %s/traces\n", fmt.Sprintf("http://localhost:%d", appdashPort))
	fmt.Printf("- Network: %s/traces\n", fmt.Sprintf("http://%s:%d", tools2.GetLocaHonst(), appdashPort))
